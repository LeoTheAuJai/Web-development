...
if(isset($_POST["delete"]))
{
    ...
    $sw_id=$_POST["student_work_id"];
    $stmt = $conn->prepare("DELETE FROM studentworks WHERE sw_id = ?");
    $stmt->bind_param("i",$sw_id );
    if ($stmt->execute()) {
        echo '<script>
        alert("Deleted student work ID: '.$sw_id.' successfully!");
...
if (isset($_POST["edit"])) {
    ...
    $stmt = $conn->prepare("UPDATE studentworks SET logo_name=?, logo_type=?, file_name=?, file_type=?, title=?, description=?, student_ID=?, course_ID=? WHERE sw_id=?");

    $stmt->bind_param("ssssssssi", $logoName, $fileExtension1, $fileName, $fileExtension2, $title, $description,$author_user_ID, $courseID,$sw_id);
